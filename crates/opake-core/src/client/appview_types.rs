...
///
/// The AppView indexes full keyring data from the firehose so clients
/// don't need raw XRPC listRecords calls.
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct AppviewKeyring {
    pub uri: String,
    pub owner_did: String,
    pub rotation: u64,
    #[serde(default)]
...
pub struct KeyringsResponse {
    pub keyrings: Vec<AppviewKeyring>,
    pub cursor: Option<String>,
}
...
/// A directory record from the AppView tree/sync endpoints.
#[derive(Debug, Clone, Deserialize, Serialize)]
...
/// A document record from the AppView tree/sync endpoints.
#[derive(Debug, Clone, Deserialize, Serialize)]
...
/// The AppView stores only metadata — the full record (blob ref, encrypted
...
impl TreeDirectory {
    /// Convert an AppView directory into a `CachedRecord` for local storage.
    ///
...
impl TreeDocument {
    /// Convert an AppView document into a `CachedRecord` for local storage.
    pub fn to_cached_record(&self) -> crate::storage::CachedRecord {
        let value = serde_json::json!({
            "opakeVersion": 1,
