// Auth signing for appview requests.
//
...
// that the appview's auth middleware expects.

use base64::engine::general_purpose::STANDARD as BASE64;
use base64::Engine;
use ed25519_dalek::{Signer, SigningKey};

/// Build the signed Authorization header value for an appview request.
///
/// Signature covers: `<method>:<path>:<timestamp>:<did>`
/// Returns: `Opake-Ed25519 <did>:<timestamp>:<base64(signature)>`
pub fn sign_appview_request(
    method: &str,
    path: &str,
    did: &str,
    signing_key: &[u8; 32],
...
mod tests {
    ...
    fn roundtrip_signature_verifies() {
        ...
        let header = sign_appview_request("GET", "/api/inbox", "did:plc:abc", &secret, 1709330400);
    ...
    fn header_format_is_correct() {
        let secret = [1u8; 32];
        let header = sign_appview_request("GET", "/api/inbox", "did:plc:test", &secret, 12345);
    ...
    fn different_inputs_produce_different_signatures() {
        let secret = [7u8; 32];
        let sig_a = sign_appview_request("GET", "/api/inbox", "did:plc:a", &secret, 100);
        let sig_b = sign_appview_request("GET", "/api/inbox", "did:plc:b", &secret, 100);
        let sig_c = sign_appview_request("POST", "/api/inbox", "did:plc:a", &secret, 100);
        let sig_d = sign_appview_request("GET", "/api/inbox", "did:plc:a", &secret, 200);
