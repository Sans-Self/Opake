...
/// directories. Validates the collection is `at.opake.directory`.
...
mod tests {
    ...
    async fn happy_path() {
        ...
        let uri = format!("at://{TEST_DID}/at.opake.directory/dir1");
    ...
    async fn rejects_root_deletion() {
        ...
        let uri = format!("at://{TEST_DID}/at.opake.directory/self");
    ...
    async fn rejects_non_empty_directory() {
        let directory = dummy_directory_with_entries(
            "Photos",
            vec!["at://did:plc:test/at.opake.document/doc1".into()],
        );
        let uri = format!("at://{TEST_DID}/at.opake.directory/dir1");
        let mock = MockTransport::new();
        mock.enqueue(get_record_response(&uri, &directory));
    ...
    async fn rejects_wrong_collection() {
        let mock = MockTransport::new();
        let mut client = mock_client(mock);
        let uri = format!("at://{TEST_DID}/at.opake.document/abc");
    ...
    async fn pds_404_on_fetch() {
        let uri = format!("at://{TEST_DID}/at.opake.directory/gone");
        let mock = MockTransport::new();
        mock.enqueue(HttpResponse {
            status: 404,
            headers: vec![],
