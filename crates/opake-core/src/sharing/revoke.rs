...
/// `app.opake.cloud.grant` to prevent accidental deletion of other
/// record types.
...
mod tests {
    ...
    async fn happy_path() {
        ...
        let uri = format!("at://{}/app.opake.cloud.grant/tid123", TEST_DID);
    ...
    async fn pds_404() {
        ...
        let mut client = mock_client(mock);
        let uri = format!("at://{}/app.opake.cloud.grant/gone", TEST_DID);
        let err = revoke_grant(&mut client, &uri).await.unwrap_err();
