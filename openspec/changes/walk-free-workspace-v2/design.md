...
- **Endorsement is pre-fork-scoped.** Count only distinct members who were already managers at the fork's common ancestor. A naive "distinct branch authors" metric is Sybil-able: a manager adds K sockpuppets, has each author a trivial supersede, and out-endorses an honest branch. Scoping to the pre-fork roster gives the sockpuppets zero weight, because they were not members when the contest began. (Only a current manager can mint them at all, so even the naive gap reduced to the manager-vs-manager case; pre-fork scoping closes it outright.)

- **The tie-break is a VRF, not the CID.** The obvious "lowest CID" tie-break is *grindable*: a CID is a hash of author-chosen bytes, so an author pads or reorders content and mines a low CID — accidental reverse proof-of-work, won by whoever computes more hashes. No hash of the record fixes this, because any deterministic function of attacker-controlled bytes is mineable. The fix is to move the tie-break's entropy off the record entirely: each fork-eligible record carries a VRF (verifiable random function) output over the shared parent CID, computed with the author's roster-carried VRF key. A VRF output is unique per `(key, input)` — the author cannot search for a favourable one — and every observer verifies it against the fork-base roster and computes the same winner offline. The residual influence (mint a low-VRF identity and add it) is a witnessed membership change with zero pre-fork endorsement, so it loses before the tie-break is even consulted. Losing a tie costs a retry (D4) and a tie dies the moment a real member endorses either branch, so the tie-break governs only the pre-endorsement instant — the VRF makes even that unbiasable.
...
- **Small-n tie-breaks — documented, not solved.** The two-manager knife fight and all-managers-dead succession — no-owner + enforceable roles + no-coordination is jointly unsatisfiable at 2–3 people. Discard-and-retry makes losing cheap but does not create a quorum where there are only two principals. The convergence simulation showed the model's residual attacks (endorsement Sybil, tie-break grinding, retry starvation) all require a competing *manager* and concentrate here; pre-fork endorsement and the VRF tie-break (D8) narrow them to "a manager who wins every real-time race, bounded by relenting", but they do not manufacture a quorum. Accepted as a documented limitation of the trust model, not a solved problem (`spec:workspace § Stated limitations no construction removes`).
