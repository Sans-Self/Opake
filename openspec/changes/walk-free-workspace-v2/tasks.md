...
Federation-class change. Nothing past group 1 begins until the spec delta is reviewed and signed off and the CID prerequisite has landed. The phases are ordered so each layer above the trustless core is shippable dark and reversible on its own.
...
- [ ] 1.2 The delta is reviewed and signed off — the federation-class requirement — before any code. The following decisions are folded into the delta: removal-rollback durability (built-upon floor plus indexer ceiling), directory fork tie-break by VRF, roster key acquisition via a once-only `publicKey/self` read pinned in the roster, sequencer key provenance (a roster-attested role key), re-add key rebinding, and displaceable confirmation. Non-blocking note for sync: `keyring-tombstones § Rollback restores the newest live record and re-broadcasts it` — the heading says "newest" over an endorsement-selected body; carry an OpenSpec RENAMED op at sync or leave the wart.
- [x] 1.3 Cross-spec and consistency review of the delta; surviving findings folded — removal durability, head-selection endorsement and tie-break, and the sibling reconciliations in `auth-pairing`, `background-work`, and `document-crypto`
