...
- **THEN** M may reappear — this is the named first-fold durability window (`spec:workspace § Stated limitations no construction removes`), which is why a manager does not treat the removal as durable until it is built upon (`spec:workspace-membership § Removal is durable once built upon, not merely witnessed`); the revert is detected and re-issued, not a silent reversal the design claims to prevent
