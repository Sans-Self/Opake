...
A rollback SHALL NOT reinstate a member whose removal is carried forward by any live record. Because the restored head is the endorsement-selected head over what remains live, deleting a record only reverses a removal when no surviving record still states that removal — which is exactly the accepted durability window (`spec:workspace-membership § Removal is effective when witnessed`): a removal witnessed and built upon lives in descendants that a single delete cannot erase, while a removal that existed only in the deleted record was never witnessed and its reversal is the same first-fold loss the change names rather than removes. A removing manager therefore SHALL NOT treat a rotation as complete until the removal is independently witnessed, so that a later delete of the removal record cannot silently reinstate the member.
