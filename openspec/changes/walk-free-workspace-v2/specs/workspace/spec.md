...
- **Removal is the knife's edge.** It is the one operation where cryptography (rotate the key) and agreement (everyone accepts the new roster) must land together, and it is where the durability window bites (`spec:workspace-membership § Removal is effective when witnessed`).
