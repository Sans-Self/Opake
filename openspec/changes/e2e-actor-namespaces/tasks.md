...
- [ ] 1.1 Add `E2E_ACTOR_NS` handling: read + validate (`[a-z0-9-]{1,16}`, fail fast
...
- [ ] 1.2 Make `fixtures.ts` workerIndex→actor mapping namespace-aware (role +
      PDS placement mirrored from the checked-in set; handles `<role>-<ns>.<pds-host>`)
- [ ] 1.3 Thread `nsPaths()` through `playwright.config.ts` (`outputDir`, HTML
      reporter folder) and `auth.setup.ts` (storageState locations); bare paths when
      no namespace is set

## 2. Deterministic provisioning

- [ ] 2.1 Implement mnemonic derivation: BIP-39 entropy = SHA-256(`opake-e2e:<ns>:<role>`),
      24 words, with a unit test pinning derivation stability
- [ ] 2.2 Extend `pds-admin.ts` with idempotent account creation for a namespaced
      actor (resolve handle → skip if live, else admin invite-code registration)
- [ ] 2.3 Provision in `auth.setup.ts`: create account if missing, import derived
      identity, publish public-key record — same guarantees as bootstrap actors
- [ ] 2.4 Federation test citing dev-env § Deterministic actor fixtures: provision a
...
- [ ] 3.1 Replace the TTL gate in `auth.setup.ts` with the liveness probe: load
...
- [ ] 3.2 Test citing e2e-testing § Web authentication is a fixture, not a flow
...
- [ ] 4.1 Add namespace argument to justfile e2e recipes (`just e2e-web [ns]`,
      federation equivalent where fixture actors are resolved); bare invocations
      byte-identical to today
- [ ] 4.2 Isolation test citing e2e-testing § Parallel workers do not share mutable
...
- [ ] 4.3 Run the default-path suite once with no namespace set to prove behavior is
      unchanged (same paths, same actors, green)

## 5. Namespace teardown

- [ ] 5.1 Implement deprovisioning in `pds-admin.ts`: delete a namespace's accounts
...
- [ ] 5.2 Justfile recipe (`just e2e-ns-clean <ns>`) wrapping deprovisioning;
      refuses the empty/default namespace
- [ ] 5.3 Test citing dev-env § Deterministic actor fixtures (deprovision scenario):
...
- [ ] 6.1 Update tests README / dev-env docs with the namespace knob, derivation
      scheme, teardown recipe, and the retirement of the `.auth` TTL heuristic
- [ ] 6.2 `just spec-lint` green; new citations resolve; remove any now-dead TTL
