...
#### Scenario: a host that rewrites both records is still rejected

- **GIVEN** a verified account whose host serves both a minted pair response and a matching rewritten `publicKey/self`
- **WHEN** the new device completes the pair
