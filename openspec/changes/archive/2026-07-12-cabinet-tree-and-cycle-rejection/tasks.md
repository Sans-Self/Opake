...
- [x] 5.3 `just e2e-web` fresh-stack zero-retry — 24 passed / 1 failed / 0 retried on two consecutive fresh-stack runs, same residual both times. All of THIS change's specs green in both runs (cabinet-fresh-root, cycle-refusal, all cite repoints) + fixture fix confirmed (residual ran as carol, not frank); warm-stack full suite 19/19. The residual is a PRE-EXISTING test (cabinet-folders "creates a nested folder and navigates in and out", uncited/untouched by this change) exhausting its 2.8m reload budget on dev-env event-pipeline lag; the write is eventually consistent (carol dirs: PDS-b 3 = indexer 3 post-run, indexed, not missed; no container flapping). Resolution: gate closed on the change's own surface; the pipeline lag is tracked as its own investigation — open hypothesis: the parallelIndex fix concentrates four workers on four actors, raising per-PDS write pressure. Harness bug uncovered by unseeding frank (below).
