...
The invitation machinery is half-dead weight: the lexicons declare two invitation types but `"share"` was never built (no mint or redemption path, key handoff undesigned), and even the workspace type has no working product loop — the web dialog was removed because it generated links to a route that never existed, `acceptInvitation` has zero callers, and owner-side acceptance discovery was never designed. Rather than carry unbuilt wire-format promises, the whole surface goes. Separately, `app.opake.grant.expiresAt` has no writer and no enforcer, yet reads like a security guarantee. Both removals are cheap to reverse as designed features later.

## What Changes

- **BREAKING (lexicon):** `app.opake.invitation` and `app.opake.invitationAcceptance` are deleted. No working redemption loop existed, so nothing breaks in practice.
- **BREAKING (lexicon):** `app.opake.grant` loses the `expiresAt` field. No code path ever wrote it.
...
- `app.opake.authFullAccess` permission set drops the invitation collections.
...
- **Lexicons:** delete `app.opake.invitation.json`, `app.opake.invitationAcceptance.json`; edit `app.opake.grant.json` (remove `expiresAt`), `app.opake.authFullAccess.json` (drop invitation collections).
...
- **Indexer:** no impact — invitations were never indexed. `scope.rs` did list both invitation collections in `OPAKE_COLLECTIONS` (and its compile-time coverage test); both are removed, so the OAuth scope string drops the two `repo:app.opake.invitation*` grants.
