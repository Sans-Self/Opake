...
- Rotation after leave: forward secrecy against a leaver requires a manager to rotate afterwards. Should a manager's daemon rotate automatically on observing a leave? Related to auto key rotation (#90). The rotation event itself is synchronous and self-sufficient (`spec:workspace-key-rotation § The rotation event is synchronous and self-sufficient`); the only open matter here is trigger policy.
