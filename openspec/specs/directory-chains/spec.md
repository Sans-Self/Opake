...
### Requirement: Concurrent supersedes fork, and the loser replays against the winner

When two curators supersede the same prior canonical concurrently, the chain forks: both records exist and both name the same `supersedes` target. The indexer SHALL detect the fork (a record whose `supersedes` target already has a successor), pick the winner deterministically by `createdAt` with a `(did, rkey)` tiebreak, keep the loser's record on its PDS but out of the canonical chain, and emit a `chain:forked` SSE event scoped to the affected workspace and chain. The losing client SHALL refetch the new canonical, replay its intended change on top, and write a fresh supersede.

Fork detection operates on the `supersedes` back-edge and needs no plaintext path, which is why it holds even though directory paths are encrypted-name-derived. The event carries the loser's URI, the fork point, and the winner's URI + CID (`SseChainForked`, crates/opake-core/src/indexer/sse/events.rs). Fan-out is stateless (crates/opake-core/src/indexer/chain_fork_keeper.rs); the replay-on-fork behavior lives in the client/React layer, not core.
...
- **THEN** one wins by `createdAt`/`(did, rkey)`, the other receives a `chain:forked` event and replays its add against the new head
...
- Replay-on-fork implementation. The `chain:forked` signal, its event type, and the stateless fan-out keeper exist and are tested; the actual refetch-and-replay loop is described as a client/React hook and I did not verify a concrete implementation. Whether every curatorial entry point (add, delete, rename, move, substitute) has a replay path, or only some, is unconfirmed.
...
- Cabinet (personal, non-workspace) directory trees. Those use a `self`-rkey root and direct key wrapping with no supersede chain, additivity check, or fork handling; this spec covers workspace directory chains.
