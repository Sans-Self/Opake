...
| Key-rotation re-wrap sweep | CLI daemon (committed) | *(incoming — key-rotation change)* keyring wraps whose rotation trails the current head | CAS (`swapRecord`) on the in-place re-wrap; targets re-resolved per item at write time | The first consumer of `put_record_conditional`. Cites this contract rather than re-deriving it. |
