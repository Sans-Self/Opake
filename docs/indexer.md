...
`chain:forked` fires on the workspace topic when a supersede points at a URI that is no longer the head — two members wrote against the same head and one lost. The payload is flat, not a record envelope: `{workspace_id, scope, path, your_uri, fork_point_uri, winner_uri, winner_cid}`, where `scope` is `keyring` or `directory`. The loser's record is persisted but the chain does not advance. The indexer's job ends at detection and fan-out: what the losing client does with the event — refetch, replay its intent, or surface the conflict — is deliberately left to the client, and no automatic replay exists today.
