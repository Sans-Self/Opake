...
The sweep is shaped as a background task under the [background-work contract](BACKGROUND_WORK.md): its work set is derived from records (documents whose `keyringRef.rotation` trails the head), each re-wrap is a single CAS-conditioned write, duplicate or interrupted runners re-derive the remainder, and completion is never required for any guarantee. `crates/opake-core/src/rewrap.rs` holds the planner. No daemon tier currently schedules it: a member may be admitted without a current wrap, and replacing a document's sole wrap would strip such a historical-only reader of access they legitimately hold. It stays unscheduled until the write boundary can exclude per item against the fresh keyring head.
