  ...
  return (
    <div className="flex flex-1 flex-col gap-6 overflow-auto p-6">
      ...
      <div className="text-base-content/60 space-y-1 text-sm">
        {did && <p>DID: <span className="font-mono text-xs">{did}</span></p>}
        {handle && <p>Handle: <span className="font-mono text-xs">{handle}</span></p>}
        {pdsUrl && <p>PDS: <span className="font-mono text-xs">{pdsUrl}</span></p>}
      </div>
