...
<Lead>
  A grant is an `app.opake.grant` record on the sharer's PDS that wraps a document's content key
...
DID document, PDS discovery), and fetches the target's `app.opake.publicKey/self` record. The
...
PDS is reachable) but there's no `app.opake.publicKey/self` record, you get `RecipientNotReady`.
...
`createPendingShare` writes an `app.opake.pendingShare` record on your PDS. The daemon
