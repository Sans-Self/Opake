...
{/* update-needed(#25): the `listInboxSnippet` comment names `ownerDid` — InboxGrant has no such field; the sharer's DID is `authorDid` (types.ts), which the snippet's own log line already uses. */}
