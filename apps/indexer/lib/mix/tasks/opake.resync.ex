defmodule Mix.Tasks.Opake.Resync do
  ...
  every DID the indexer has ever seen as a workspace member).
  """

  use Mix.Task

  @shortdoc "Backfill opake records from a PDS into the indexer"
  ...
  def run(argv) do
    ...
    cond do
      opts[:all] ->
        OpakeIndexer.Backfill.backfill_known_dids()
      ...
      length(args) == 1 ->
        ...
        case OpakeIndexer.Backfill.backfill_did(did) do
          :ok ->
