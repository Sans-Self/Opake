defmodule OpakeIndexer.FirehoseTest do
  use OpakeIndexer.DataCase, async: true

  alias OpakeIndexer.Firehose
  alias OpakeIndexer.Firehose.State
  alias OpakeIndexer.Queries.{CursorQueries, GrantQueries, KeyringQueries, DirectoryQueries}
  ...
  test "indexes grant create" do
    Firehose.process_message(grant_create_json(), 0)
  ...
  test "indexes grant delete" do
    Firehose.process_message(grant_create_json(), 0)
    Firehose.process_message(grant_delete_json(), 1)
  ...
  test "indexes keyring create" do
    Firehose.process_message(keyring_create_json(["did:plc:alice", "did:plc:bob"]), 0)
  ...
  test "indexes keyring update replaces members" do
    Firehose.process_message(keyring_create_json(["did:plc:alice", "did:plc:bob"]), 0)
    Firehose.process_message(keyring_update_json(["did:plc:alice", "did:plc:charlie"]), 1)
  ...
  test "indexes keyring delete" do
    Firehose.process_message(keyring_create_json(["did:plc:alice"]), 0)
    Firehose.process_message(keyring_delete_json(), 1)
  ...
  test "indexes cabinet directory (directKeyWrapping)" do
    ...
    Firehose.process_message(directory_create_json(key_wrapping), 0)
  ...
  test "soft-deletes directory on delete event" do
    ...
    Firehose.process_message(directory_create_json(key_wrapping), 0)
    Firehose.process_message(directory_delete_json(), 1)
  ...
  test "indexes cabinet document (directEncryption)" do
    ...
    Firehose.process_message(document_create_json(encryption), 0)
  ...
  test "soft-deletes document on delete event" do
    ...
    Firehose.process_message(document_create_json(encryption), 0)
    Firehose.process_message(document_delete_json(), 1)
  ...
  describe "cursor save monotonicity" do
    setup do
      ...
      prev_interval = Application.get_env(:opake_indexer, :cursor_save_interval_ms)
      Application.put_env(:opake_indexer, :cursor_save_interval_ms, 0)
      ...
      on_exit(fn ->
        if prev_interval do
          Application.put_env(:opake_indexer, :cursor_save_interval_ms, prev_interval)
        else
          Application.delete_env(:opake_indexer, :cursor_save_interval_ms)
        end
      end)
    ...
    test "advances the cursor for strictly-increasing time_us" do
      Firehose.process_message(grant_json_with_time(1_000_000), 0)
      assert State.last_cursor_time_us() == 1_000_000

      Firehose.process_message(grant_json_with_time(2_000_000), 1)
      assert State.last_cursor_time_us() == 2_000_000

      Firehose.process_message(grant_json_with_time(5_000_000), 2)
      assert State.last_cursor_time_us() == 5_000_000
    end

    test "ignores an older time_us after a newer one (no rollback)" do
      Firehose.process_message(grant_json_with_time(5_000_000), 0)
      assert State.last_cursor_time_us() == 5_000_000

      # Older event arrives — cursor must stay at the high-water mark.
      Firehose.process_message(grant_json_with_time(3_000_000), 1)
      assert State.last_cursor_time_us() == 5_000_000
    ...
    test "ignores an equal time_us (strictly monotonic)" do
      ...
      Firehose.process_message(grant_json_with_time(5_000_000), 1)
      assert State.last_cursor_time_us() == 5_000_000
    end
  end
end
