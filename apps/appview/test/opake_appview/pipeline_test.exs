defmodule OpakeAppview.PipelineTest do
  @moduledoc """
  End-to-end pipeline tests: raw Jetstream JSON → Indexer.process_message → DB → query results.
  """
  ...
  test "keyring create with roles indexes members correctly" do
    ...
    members =
      Repo.all(
        from(km in OpakeAppview.Schemas.KeyringMember,
  ...
  test "keyring create indexes full record into keyrings table" do
    ...
    keyring = Repo.get(OpakeAppview.Schemas.Keyring, keyring_uri)
    assert keyring != nil
    ...
    members =
      from(km in OpakeAppview.Schemas.KeyringMember, where: km.keyring_uri == ^keyring_uri)
      |> Repo.all()
  ...
  test "keyring delete removes from both keyrings and keyring_members" do
    ...
    Indexer.process_message(create_json, 0)

    assert KeyringQueries.is_member?(keyring_uri, "did:plc:alice")
    assert Repo.get(OpakeAppview.Schemas.Keyring, keyring_uri) != nil
    ...
    Indexer.process_message(delete_json, 1)
