defmodule OpakeAppview.Jetstream.Consumer do
  ...
  routes every received frame through `OpakeAppview.Indexer`.
  ...
  Controlled by the `:compression` config key:
    ...
    * `:zstd` (default) — request zstd-compressed frames via the
      ...
      `OpakeAppview.Jetstream.Compression` using a vendored dictionary.
  ...
  def handle_frame({:binary, msg}, %__MODULE__{compression: :zstd} = state) do
    ...
    case Compression.decompress(state.decompression_context, msg) do
      {:ok, json} ->
        event_count = Indexer.process_message(json, state.event_count)
  ...
  defp firehose_mode do
    Application.get_env(:opake_appview, :firehose_mode, :full)
  end

  defp compression_mode do
    Application.get_env(:opake_appview, :compression, :zstd)
  end
